...
public class PromptForGit {
  ...
  public static void prompt(
      String purpose, String defaultID, IGistPromptCompletionListener listener) {
    Platform.runLater(
        () -> {
          ...
          if (result.isPresent()) {
            ...
            if (result.get().endsWith(".git")) gistcode = result.get();
            else
              gistcode =
                  "https://gist.github.com/" + ScriptingEngine.urlToGist(result.get()) + ".git";
