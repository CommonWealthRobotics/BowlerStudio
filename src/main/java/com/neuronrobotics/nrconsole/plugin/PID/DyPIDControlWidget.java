...
public class DyPIDControlWidget extends JPanel implements IDyIOEventListener {
  ...
  private void populateDyPID() {
    ...
    if (conf.getInputChannel() == 0xff) inChan.setNoneItemSelected();
    else inChan.setSelectedInteger(conf.getInputChannel());
    if (conf.getOutputChannel() == 0xff) outChan.setNoneItemSelected();
    else outChan.setSelectedInteger(conf.getOutputChannel());
  ...
  private void updateInChan() {
    ...
    if (inChan.getSelectedItem() == null) return;
    // System.out.println("Input channel set to "+inChan.getSelectedItem() );
    int chan = inChan.getSelectedInteger();
    if (chan < 24 && chan > 0) {
      Collection<DyIOChannelMode> m =
          getAvailableInputModes(widgit.getGui().getDyio().getChannel(chan).getAvailableModes());
  ...
  private void updateOutChan() {
    ...
    if (outChan.getSelectedItem() == null) return;
    // System.out.println("Output channel set to "+outChan.getSelectedItem() );
    int chan = outChan.getSelectedInteger();
    if (chan < 24 && chan > 0) {
      Collection<DyIOChannelMode> m =
          getAvailableOutputModes(widgit.getGui().getDyio().getChannel(chan).getAvailableModes());
      for (DyIOChannelMode mode : m) {
  ...
  private void initDyPID() {
    ...
    inChan.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent arg0) {
            updateInChan();
          }
        });
    outChan.addActionListener(
        new ActionListener() {
    ...
    DypidSet.addActionListener(
        new ActionListener() {

          public void actionPerformed(ActionEvent arg0) {
            if (inChan.getSelectedItem() != null && outChan.getSelectedItem() != null) {
              widgit.getPidSet().setEnabled(true);
              DyIOChannelMode o = (DyIOChannelMode) outMode.getSelectedItem();
              if (o == null) o = DyIOChannelMode.OFF;
              DyIOChannelMode i = (DyIOChannelMode) inMode.getSelectedItem();
              if (i == null) i = DyIOChannelMode.OFF;
              DyPIDConfiguration config =
                  new DyPIDConfiguration(
                      widgit.getGroup(),
              ...
              JOptionPane.showMessageDialog(
                  null,
                  "DyIO Channel/Modes are not all set",
                  "DyPID ERROR",
