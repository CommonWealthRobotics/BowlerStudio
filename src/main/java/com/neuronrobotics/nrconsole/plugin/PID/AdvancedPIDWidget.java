...
public class AdvancedPIDWidget extends JPanel {
          ...
          public void actionPerformed(ActionEvent arg0) {
            ...
            try {
              ...
              JOptionPane.showMessageDialog(
                  null, "Bad Step values", "PID Error", JOptionPane.ERROR_MESSAGE);
              return;
            }
  ...
  private class SineTrack extends Thread {
    ...
    public SineTrack(int amplitude, double period, int pointsPerCycle, int cycles) {
      ...
      cy = cycles;
      run = true;
      // System.out.println("Amplitude of sin wave: "+a+" period: "+p+" cycles/sec, points per
      // cycle: "+pPC+" for "+cy+" cycles");
  ...
  private class StepLoop extends Thread {
    ...
    public void run() {
      for (int i = 0; i < iter; i++) {
        if (run) getPid().setSetpoint((int) start);
        if (run)
          try {
            Thread.sleep((long) (time * 1000));
          } catch (InterruptedException e) {
          }
        if (run) getPid().setSetpoint((int) middle);
        if (run)
          try {
            Thread.sleep((long) (time * 1000));
          } catch (InterruptedException e) {
          }
        if (run) getPid().setSetpoint((int) end);
        if (run)
          try {
            Thread.sleep((long) (time * 1000));
          } catch (InterruptedException e) {
