...
public class ScriptingFileWidget extends BorderPane implements IFileChangeListener {
    ...
    try {
      image.setImage(
          AssetFactory.loadAsset(
              "Script-Tab-" + ScriptingEngine.getShellType(currentFile.getName()) + ".png"));
    } catch (Exception e) {
  ...
  private ScriptingFileWidget(ScriptingWidgetType type) {
        ...
        e -> {
          new Thread() {
            public void run() {
              ...
              if (langaugeType.getIsTextFile()) save();
    ...
    fileNameBox
        .textProperty()
        .addListener(
            (ov, prevText, currText) -> {
              // Do this in a Platform.runLater because of Textfield has no padding at first time
              // and so on
              Platform.runLater(
                  () -> {
                    Text text = new Text(currText);
                    text.setFont(
                        fileNameBox.getFont()); // Set the same font, so the size is the same
                    double width =
                        text.getLayoutBounds().getWidth() // This big is the Text in the TextField
                            + fileNameBox.getPadding().getLeft()
                            + fileNameBox
                                .getPadding()
                                .getRight() // Add the padding of the TextField
                            + lengthScalar; // Add some spacing
                    fileNameBox.setPrefWidth(width); // Set the width
                    fileNameBox.positionCaret(
                        fileNameBox
                            .getCaretPosition()); // If you remove this line, it flashes a little
                    // bit
                  });
            });
    fileListBox
            ...
            (ov, prevText, currText) -> {
              ...
              Platform.runLater(
                  () -> {
                    Text text = new Text(currText);
                    text.setFont(
                        fileListBox.getFont()); // Set the same font, so the size is the same
                    double width =
                        text.getLayoutBounds().getWidth() // This big is the Text in the TextField
                            + fileListBox.getPadding().getLeft()
                            + fileListBox
                                .getPadding()
                                .getRight() // Add the padding of the TextField
                            + lengthScalar; // Add some spacing
                    fileListBox.setPrefWidth(width); // Set the width
                    fileListBox.positionCaret(
                        fileListBox
                            .getCaretPosition()); // If you remove this line, it flashes a little
                    // bit
                  });
  ...
  public void loadCodeFromFile(File currentFile) throws IOException {
    ...
    if (!langaugeType.getIsTextFile()) setCode("Binary File");
    else setCode(new String(Files.readAllBytes(currentFile.toPath())));
  }
      ...
      if (!currentFile.getName().contentEquals("csgDatabase.json")) {
        ...
        for (String s : f) {
          if (s.contentEquals("csgDatabase.json")) {
            ...
            if (!CSGDatabase.getDbFile().equals(dbFile)) CSGDatabase.setDbFile(dbFile);
            ...
            try {
              ...
              Platform.runLater(
                  () -> {
                    ...
                    if (stackTrace.contains("dyio")) message += "dyio";
                    else if (stackTrace.contains("camera")) message += "camera";
                    else if (stackTrace.contains("gamepad")) message += "gamepad";
                    else message += stackTrace;
                    alert.setHeaderText(message);
                    alert.showAndWait();
                    if (stackTrace.contains("dyio")) ConnectionManager.addConnection();
  ...
  private void setUpFile(File f) {
    ...
    try {
      ...
      Platform.runLater(
          () -> {
            ...
            // the use of text field for static dats is so the user cna copy the vlaues and use them
            // in their scritpts
            fileNameBox
                .textProperty()
                .addListener(
    ...
    if (!langaugeType.getIsTextFile()) return;
    try {
                ...
                Platform.runLater(
                    () -> {
                      try {
                        ...
                        if (content.length()
                            > 2) // ensures tha the file contents never get wiped out on the user
                        setCode(content);
