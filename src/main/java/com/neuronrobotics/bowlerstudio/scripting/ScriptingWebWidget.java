...
public class ScriptingWebWidget extends BorderPane implements ChangeListener<Object> {
  ...
  public ScriptingWebWidget(ScriptingWidgetType type) {
    ...
    edit.setOnAction(
        e -> {
          new Thread() {
            public void run() {
              if (isOwnedByLoggedInUser) BowlerStudio.createFileTab(currentFile);
              else {
                // todo fork git repo
  ...
  public void loadCodeFromGist(String a, WebEngine e) throws Exception {
    ...
    // }
    if (!fileList.isEmpty()) loadGitLocal(currentGit, fileList.get(0));
  ...
  @Override
  public void changed(
      @SuppressWarnings("rawtypes") ObservableValue observable, Object oldValue, Object newValue) {
