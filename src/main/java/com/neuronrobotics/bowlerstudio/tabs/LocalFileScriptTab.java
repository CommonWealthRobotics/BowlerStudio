...
public class LocalFileScriptTab extends VBox
    implements IScriptEventListener, EventHandler<WindowEvent> {
  private static final UncaughtExceptionHandler ISSUE_REPORTING_EXCEPTION_HANDLER =
        ...
        mouseEvent -> {
          ...
          SwingUtilities.invokeLater(
              () -> {
                resizeEvent();
                setSelectedTab(this);
                //				spscrollPane.setSize((int) spscrollPane.getWidth(), (int)
                // spscrollPane.getHeight());
    ...
    if (current.length() > 3 && !textArea.getText().contentEquals(current)) { // no
      ...
      SwingUtilities.invokeLater(
          () -> {
            ...
            if (previous == null) SwingUtilities.invokeLater(() -> textArea.setCaretPosition(0));
          });
    }
  }
