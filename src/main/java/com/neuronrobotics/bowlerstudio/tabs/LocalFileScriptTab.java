...
public class LocalFileScriptTab extends VBox implements IScriptEventListener, EventHandler<WindowEvent> {
    ...
    public LocalFileScriptTab(File file) throws IOException {
        ...
        sn.setOnMouseEntered(mouseEvent -> resizeEvent());
        ...
        highlighter.removeAllHighlights();
        focusedProperty().addListener((w, o, n) -> System.err.println("Focused " + file));
    ...
    private void resizeEvent() {
        SwingUtilities.invokeLater(() -> {
            sp.setSize(sp.getWidth() - 1, sp.getHeight() - 1);
            textArea.requestFocusInWindow();
            textArea.invalidate();
            textArea.repaint();
            sp.invalidate();
            sp.repaint();
            Platform.runLater(() -> sn.requestFocus());
        });
