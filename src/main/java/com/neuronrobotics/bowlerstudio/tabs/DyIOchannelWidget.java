...
public class DyIOchannelWidget {
  ...
  public void setChannel(DyIOChannel c) {
                    ...
                    new Thread() {
                      public void run() {
                        ...
                        try {
                          ...
                          if (srv.getChannel().getCachedMode()) srv.getChannel().flush();
                        } catch (Exception ex) {
                          ex.printStackTrace();
                        }
    ...
    channel.addChannelModeChangeListener(
        newMode -> {
          setMode(newMode);
        });

    channel.addChannelEventListener(
        new IChannelEventListener() {
    ...
    Platform.runLater(
        () ->
            setListenerButton.setBackground(
  ...
  public void updateValue() {
    ...
    if (getValue() != null) {
      ...
      Data<Integer, Integer> newChart =
          new XYChart.Data<Integer, Integer>(
              (int) (startTime - System.currentTimeMillis()), current);
      Platform.runLater(() -> chanValue.setText(Integer.toString(current)));
      if (!positionSlider
          .isValueChanging()) { // only updae the slider position if the user is not sliding it
        Platform.runLater(
            () -> {
      ...
      Platform.runLater(
          () -> {
            if (series.getData().size()
                > 10) { // if you keep many more points in the graph it will lag the rendering realy
              // badly
          ...
          if (myLocalListener == null) {
            ...
            myLocalListener =
                (IChannelEventListener)
                    ScriptingEngine.inlineScriptStringRun(textArea.getText(), null, "Groovy");
  ...
  private void setUpListenerPanel() {
    ...
    textArea.setText(
        "return new IChannelEventListener() { \n"
            + "\tpublic \n"
