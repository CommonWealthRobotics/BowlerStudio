...
public class DyIOPanel implements Initializable {
  ...
  public void setDyIO(DyIO d) {
    ...
    for (int i = 0; i < 24; i++) {
      ...
      chan.addChannelEventListener(
          e -> {
            displayFlash(index, e.getValue());
          });
      channelButtonSelectors
          .get(index)
          .setOnMouseEntered(
              event -> {
                Platform.runLater(
                    () ->
                        channelButtonSelectors
                            .get(index)
                            .setImage(BowlerStudioResourceFactory.getChanHighlight()));
              });
      channelButtonSelectors
          .get(index)
          .setOnMouseExited(
              event -> {
                Platform.runLater(
                    () ->
                        channelButtonSelectors
                            .get(index)
                            .setImage(BowlerStudioResourceFactory.getChanDefault()));
              });
              ...
              return new ListCell<String>() {
                ...
                protected void updateItem(String item, boolean empty) {
                  ...
                  if (item == null || empty) {
                    ...
                    icon.setGraphic(
                        new ImageView(
                            BowlerStudioResourceFactory.getModeImage(
                                DyIOChannelMode.getFromSlug(item))));
                    setGraphic(cell);
          ...
          for (int i = 0; i < 24; i++) {
            ...
            if (controller.isFireValue()) {
              ...
              if (dyio.getMode(i) == DyIOChannelMode.SERVO_OUT
                  && dyio.getChannel(i).getCachedMode()) {
