...
public class CreatureLab extends AbstractBowlerStudioTab implements IOnEngineeringUnitsChange {
    ...
    public void initializeUI(BowlerAbstractDevice pm) {
        ...
        autoRegen.setOnAction(event -> {
            if (autoRegen.isSelected()) generateCad();
        });
    ...
    private void finishLoading(MobileBase device) {
        ...
        try {
            rootItem = new TreeItem<>(device.getScriptingName(), AssetFactory.loadIcon("creature.png"));
        } catch (Exception e) {
            rootItem = new TreeItem<>(device.getScriptingName());
        }
        ...
        tree.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            ...
            new Thread(() -> {
                ...
                if (widgetMapForTreeitems.get(treeItem) != null) {
                    ...
                    Platform.runLater(() -> tab.getControlsBox().getChildren().clear());
    ...
    private void setDefaultWalkingEngine(MobileBase device) {
        ...
        for (DHParameterKinematics dh : device.getAllDHChains()) setDefaultDhParameterKinematics(dh);
    }
