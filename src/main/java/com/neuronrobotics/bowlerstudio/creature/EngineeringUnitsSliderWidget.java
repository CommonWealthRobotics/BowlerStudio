...
public class EngineeringUnitsSliderWidget extends GridPane implements ChangeListener<Number> {
  ...
  public EngineeringUnitsSliderWidget(
      IOnEngineeringUnitsChange listener,
      double min,
      double max,
      double current,
      double width,
  ...
  public EngineeringUnitsSliderWidget(
      IOnEngineeringUnitsChange listener, double current, double width, String units) {
  ...
  public EngineeringUnitsSliderWidget(
        ...
        .addListener(
            (ChangeListener<Boolean>)
                (observable, oldValue, newValue) -> {
                  ...
                  if (!newValue) getListener().onSliderDoneMoving(this, val);
                });
    ...
    getColumnConstraints()
        .add(new ColumnConstraints(unitsString.length() * 7)); // column 2 is 100 wide
  ...
  private void setValueLocal(double value) {
    ...
    if (val > setpoint.getMax()) {
      if (isAllowResize()) setpoint.setMax(val);
      else val = setpoint.getMax();
    }
    if (val < setpoint.getMin()) {
      if (isAllowResize()) setpoint.setMin(val);
      else val = setpoint.getMin();
    }
  ...
  public String getFormatted(double value) {
    if (intCast) return String.valueOf((int) value);
