...
public class DhChainWidget extends Group implements IDeviceConnectionEventListener {
  ...
  public DhChainWidget(AbstractKinematicsNR device2, IOnEngineeringUnitsChange externalListener) {
    ...
    kinematics.setOnAction(
        event -> {
          new Thread() {

            public void run() {
              if (getKinematicsFile() == null) setKinematicsFile(ScriptingEngine.getLastFile());
              setKinematicsFile(
              ...
              File last =
                  FileSelectionFactory.GetFile(
                      currentFile == null
                          ...
                          : new File(
                              ScriptingEngine.getWorkspace().getAbsolutePath()
                                  + "/"
                                  + currentFile.getName()),
    ...
    advanced
        ...
        .add(
            new TransformWidget(
                "Base to Global",
                device.getFiducialToGlobalTransform(),
    ...
    if (mbase == null) {
      ...
      controls
          ...
          .add(
              new TransformWidget(
                  "Place Limb",
                  device.getRobotToFiducialTransform(),
                  new IOnTransformChange() {
  ...
  private void setKinematics() {
    if (getKinematicsFile() != null) {
      try {
        if (mbase != null) {
          mbase.setWalkingDriveEngine(
              (IDriveEngine) ScriptingEngine.inlineFileScriptRun(getKinematicsFile(), null));
        } else if (dhdevice != null) {
          dhdevice.setInverseSolver(
              (DhInverseSolver) ScriptingEngine.inlineFileScriptRun(getKinematicsFile(), null));
        }
      } catch (Exception e) {
  ...
  public void onTabReOpening() {
    ...
    if (dhdevice != null) dhLinks = dhdevice.getChain().getLinks();
    ...
    for (int i = 0; i < device.getFactory().getLinkConfigurations().size(); i++) {
      ...
      DHLink dh = null;
      if (dhdevice != null) dh = dhLinks.get(i);
      ...
      DHLinkWidget w = new DHLinkWidget(i, dh, device, del, externalListener, manager);
      widgets.add(w);
