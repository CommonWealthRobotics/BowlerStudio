...
public class MobleBaseMenueFactory {
    ...
    public static void load(MobileBase device, TreeView<String> view, TreeItem<String> rootItem,
        ...
        callbackMapForTreeitems.put(physics, () -> widgetMapForTreeitems.computeIfAbsent(physics, k -> new Group(new CreaturePhysicsWidget(device))));

        TreeItem<String> publish;
        publish = new TreeItem<>("Publish", AssetFactory.loadIcon("Publish.png"));
        ...
        if (!(device.getGitSelfSource()[0] == null || device.getGitSelfSource()[1] == null)) {
            try {
                ...
                callbackMapForTreeitems.put(publish, () -> CommitWidget.commit(source, device.getXml()));
            } catch (Exception e) {
        ...
        TreeItem<String> regnerate = new TreeItem<>("Generate Cad", AssetFactory.loadIcon("Generate-Cad.png"));

        callbackMapForTreeitems.put(regnerate, creatureLab::generateCad);
        ...
        callbackMapForTreeitems.put(printable, () -> {
            ...
            Platform.runLater(() -> {
                ...
                new Thread(() -> {
                    ...
                    try {
                        files = baseManager.generateStls(device, baseDirForFiles);
                        Platform.runLater(() -> {
        ...
        callbackMapForTreeitems.put(makeCopy, () -> Platform.runLater(() -> {
            ...
            if (result.isPresent()) {
                ...
                new Thread(() -> {
                    ...
                    try {
                        gist = builder.create();
                        String gitURL = "https://gist.github.com/" + ScriptingEngine.urlToGist(gist.getHtmlUrl()) + ".git";
                        ...
                        System.out.println("copy walking engine Was: " + Arrays.toString(device.getGitWalkingEngine()));
    ...
    private static void setHardwareConfig(MobileBase base, LinkConfiguration conf, LinkFactory factory, TreeItem<String> rootItem,
        ...
        callbackMapForTreeitems.put(hwConf, () -> {
            ...
            widgetMapForTreeitems.computeIfAbsent(hwConf, k -> new Group(new LinkConfigurationWidget(conf, factory)));
    ...
    private static void loadSingleLink(int linkIndex, MobileBase base, TreeView<String> view, LinkConfiguration conf,
        ...
        callbackMapForTreeitems.put(link, () -> {
            // create the widget for the leg when looking at it for the first time
            widgetMapForTreeitems.putIfAbsent(link, lsw);
        ...
        callbackMapForTreeitems.put(design, () -> {
            // create the widget for the leg when looking at it for the first time
            widgetMapForTreeitems.computeIfAbsent(design, k -> new DhSettingsWidget(dhLink, dh, new IOnEngineeringUnitsChange() {
                @Override
    ...
    private static void loadSingleLimb(MobileBase base, TreeView<String> view, DHParameterKinematics dh,
        ...
        callbackMapForTreeitems.put(remove, () -> Platform.runLater(() -> {
            Alert alert = new Alert(AlertType.CONFIRMATION);
