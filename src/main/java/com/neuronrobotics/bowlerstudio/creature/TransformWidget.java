...
public class TransformWidget extends GridPane
    implements IOnEngineeringUnitsChange, EventHandler<ActionEvent> {
  ...
  public TransformWidget(String title, TransformNR is, IOnTransformChange onChange) {
    ...
    add(new Text("Tilt"), 3, 1);
    add(tilt, 4, 1);
    // Y line
    add(new Text("Y"), 0, 2);
    add(ty, 1, 2);

    add(new Text("Elevation"), 3, 2);
    add(elevation, 4, 2);
    // Z line
    add(new Text("Z"), 0, 3);
    add(tz, 1, 3);

    add(new Text("Azimuth"), 3, 3);
    add(azimeth, 4, 3);
  }
  ...
  private TransformNR getCurrent() {
    TransformNR tmp =
        new TransformNR(
            ...
            ty.getValue(),
            tz.getValue(),
            new RotationNR(tilt.getValue(), azimeth.getValue(), elevation.getValue()));
