...
public class DHLinkWidget extends Group implements IJointSpaceUpdateListenerNR {
  ...
  public DHLinkWidget(
      int linkIndex,
      DHLink dhlink,
      AbstractKinematicsNR device2,
      Button del,
      IOnEngineeringUnitsChange externalListener,
          ...
          .add(
              new TitledPane(
                  ...
                  new DhSettingsWidget(
                      dhdevice.getChain().getLinks().get(linkIndex), dhdevice, externalListener)));
    accordion
        .getPanes()
        .add(
            new TitledPane(
                ...
                new LinkConfigurationWidget(
                    abstractLink.getLinkConfiguration(), device2.getFactory(), setpoint, manager)));
    ...
    panel.add(del, 0, 0);
    panel.add(new Text("#" + linkIndex), 1, 0);
    panel.add(name, 2, 0);
    panel.add(setpoint, 3, 0);
    panel.add(accordion, 2, 1);

    getChildren().add(panel);
  }

  public void changed(
      ObservableValue<? extends Boolean> observableValue, Boolean wasChanging, Boolean changing) {}
