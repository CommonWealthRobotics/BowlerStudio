...
public class JogWidget extends GridPane
    ...
    game.setOnAction(
        event -> {
          if (getGameController() == null) {
            setGameController(
                (BowlerJInputDevice)
                    DeviceManager.getSpecificDevice(BowlerJInputDevice.class, "jogController"));
            if (getGameController() == null) {
  ...
  public void setKin(AbstractKinematicsNR kin) {
    if (!kin.isAvailable()) kin.connect();
    if (MobileBase.class.isInstance(kin)) setMobilebase((MobileBase) kin);
    this.kin = kin;
    try {
  ...
  @Override
  public void onEvent(
      Component comp, net.java.games.input.Event event, float value, String eventString) {
    ...
    if (comp.getName()
      ...
      slider = -value;
    if (Math.abs(x) < .01) x = 0;
    if (Math.abs(y) < .01) y = 0;
    if (Math.abs(rz) < .01) rz = 0;
    if (Math.abs(slider) < .01) slider = 0;
