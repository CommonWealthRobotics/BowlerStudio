...
public class LinkConfigurationWidget extends GridPane {
  ...
  public LinkConfigurationWidget(
      LinkConfiguration congiuration,
      LinkFactory factory,
      EngineeringUnitsSliderWidget slide,
      MobileBaseCadManager manager) {
    // this.index = index;
    ...
    for (String vitaminsType : Vitamins.listVitaminTypes()) {
      ...
      if (meta != null && meta.containsKey("shaft")) shaftType.getItems().add(vitaminsType);
    }

    shaftType.setOnAction(
        event -> {
    ...
    emHardwareSize.setOnAction(
        new EventHandler<ActionEvent>() {
          ...
          public void handle(ActionEvent event) {
            ...
            HashMap<String, Object> vitaminData =
                Vitamins.getConfiguration(
                    conf.getElectroMechanicalType(), conf.getElectroMechanicalSize());
                        ...
                        FxTimer.runLater(
                            ...
                            new Runnable() {
                              ...
                              public void run() {
                                ...
                                System.out.println(
                                    "Settting shaft size: " + shaftsize + " of " + shafttype);

                                Platform.runLater(
                                    () -> shaftType.getSelectionModel().select(shafttype));
                                Platform.runLater(
                                    () -> shaftSize.getSelectionModel().select(shaftsize));
                              }
                            });
                      });
                });
    ...
    for (String vitaminsType : Vitamins.listVitaminTypes()) {
      HashMap<String, Object> meta = Vitamins.getMeta(vitaminsType);
      if (meta != null && meta.containsKey("actuator")) emHardwareType.getItems().add(vitaminsType);
    }
    emHardwareType.setOnAction(
        event -> {
    ...
    upperBound =
        new EngineeringUnitsSliderWidget(
            ...
            conf.getStaticOffset(),
            conf.getUpperLimit() > 180 ? conf.getUpperLimit() : 180,
            conf.getUpperLimit(),
            150,
            "device units",
            true);
  ...
  private void setShaftType(
      Button editShaft, Button newShaft, final ComboBox<String> shaftSize, String selectedItem) {
            ...
            new Thread() {
              public void run() {
                for (Map.Entry<String, TextField> entry : valueFields.entrySet()) {
                  try {
                    Vitamins.setParameter(
                        type, id, entry.getKey(), (Object) entry.getValue().getText());
                  } catch (Exception e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                  }
