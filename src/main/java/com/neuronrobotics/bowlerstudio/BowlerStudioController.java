...
public class BowlerStudioController implements IScriptEventListener {
  ...
  private static IMobileBaseUI mbui =
      new IMobileBaseUI() {
        ...
        public void addCSG(Collection<CSG> toAdd, File source) {
          ...
          for (CSG b : toAdd) BowlerStudioController.addCsg(b);
        }
  ...
  public ScriptingFileWidget createFileTab(File file) {
    if (openFiles.get(file.getAbsolutePath()) != null
        && widgets.get(file.getAbsolutePath()) != null) {
    ...
    try {
      ...
      if (key.length() > 3
          && files.get(0).length() > 0
          && files.get(1).length() > 0) // catch degenerates
      ConfigurationDatabase.setObject("studio-open-git", key, files);

      fileTab.setContent(t);
      fileTab.setGraphic(
          AssetFactory.loadIcon(
              "Script-Tab-" + ScriptingEngine.getShellType(file.getName()) + ".png"));

      addTab(fileTab, true);
      widgets.put(file.getAbsolutePath(), t);
      fileTab.setOnCloseRequest(
          event -> {
    ...
    new Thread() {
      public void run() {
          ...
          for (StackTraceElement el : stackTrace) {
            try {
              ...
              if (el.getFileName().contentEquals(fileEngineRunByName.getName())) {
                widgets
                    .get(fileEngineRunByName.getAbsolutePath())
                    .setHighlight(el.getLineNumber(), Color.CYAN);
              }
            } catch (Exception e) {
        ...
        try {
          ...
          System.out.println(sw.toString());
          // space out the exception highlights, ensure any sub threads spawned here have time to
          // finish
          Thread.sleep(100);
        } catch (Exception e) {
          // TODO Auto-generated catch block
  ...
  public static void addUserNode(Node toadd) {
    Platform.runLater(
        () -> {
          if (toadd != null) CreatureLab3dController.getEngine().addUserNode(toadd);
        });
  }
  ...
  public static void addCsg(CSG toadd, File source) {
    Platform.runLater(
        () -> {
          if (toadd != null) CreatureLab3dController.getEngine().addObject(toadd, source);
        });
  }
    ...
    if (CSG.class.isInstance(o)) {
      ...
      javafx.scene.paint.Color color =
          new javafx.scene.paint.Color(
              Math.random() * 0.5 + 0.5, Math.random() * 0.5 + 0.5, Math.random() * 0.5 + 0.5, 1);
      double stroke = 0.5;
  ...
  public static void setCsg(CSG legAssembly, File cadScript) {
    Platform.runLater(
        () -> {
          ...
          if (legAssembly != null)
            Platform.runLater(
                () -> CreatureLab3dController.getEngine().addObject(legAssembly, cadScript));
        });
  }
