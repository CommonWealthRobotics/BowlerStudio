...
public class MobileBaseCadManager {
    ...
    public ArrayList<CSG> generateBody(MobileBase base) {
        ...
        try {
            ...
            if (showingStl) {
                //skip the regen
                for (CSG c : getBasetoCadMap().get(base)) {
                    getAllCad().add(c);
                }
            } else {
                if (!bail) {
                    ArrayList<CSG> newcad = cadEngine.generateBody(base);
                    for (CSG c : newcad) {
                        getAllCad().add(c);
                    }
                } else
                ...
                arrayList.clear();
                arrayList.addAll(getAllCad());
                new Thread(() -> {
                    localGetBaseCad(base);// load the cad union in a thread to make it ready for physics
                }).start();
            }
        } catch (Exception e) {
            BowlerStudioController.highlightException(getCadScript(), e);
        ...
        BowlerStudioController.setCsg(getBasetoCadMap().get(base), getCadScript());
        ...
        for (CSG part : allCad)
            for (String p : part.getParameters()) {
                CSGDatabase.addParameterListener(p, (arg0, arg1) -> {
                    //generateCad(); //Undo this after debugging
                });
            }
        this.allCad = allCad;
    }
