...
/**
 * Sample Skeleton for "BowlerStudioModularFrame.fxml" Controller Class You can copy and paste this
...
public class BowlerStudioModularFrame {
  ...
  void initialize() throws Exception {
      ...
      Platform.runLater(
          () -> {
            try {
              ...
              setTutorialDockNode(
                  new DockNode(webtab.getContent(), webtab.getText(), webtab.getGraphic()));
    ...
    connectionManagerDockNode =
        new DockNode(
            ConnectionManager.getConnectionManager().getContent(),
    ...
    creatureLab3dDockNode =
        new DockNode(threed, "Creature Lab", AssetFactory.loadIcon("CreatureLab-Tab.png"));
    ...
    Platform.runLater(
        () -> {
          if (!isOpen.get(key)) {
            ...
            else connectionManagerDockNode.dock(dockPane, DockPos.BOTTOM, getTutorialDockNode());
    ...
    if (!isOpen.get(key)) {
      ...
      Platform.runLater(
          () -> {
            ...
            else terminalDockNode.dock(dockPane, DockPos.BOTTOM, getTutorialDockNode());
                ...
                Platform.runLater(
                    () -> {
                      try {
                        ...
                        } else BowlerStudio.printStackTrace(e); // fail and show
                        // user
                      } catch (Exception e) {
                        isOpen.put(key, false);
  ...
  public void setSelectedTab(Tab tab) {
    if (webTabs.get(tab) != null) Platform.runLater(() -> webTabs.get(tab).requestFocus());
  }
}
