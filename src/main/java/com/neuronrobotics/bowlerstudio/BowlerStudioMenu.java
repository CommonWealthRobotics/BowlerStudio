...
public class BowlerStudioMenu implements MenuRefreshEvent {
    ...
    public void onMobileBaseFromGist(ActionEvent event) {
        PromptForGit.prompt("Select a Creature From a Gist", "bcb4760a449190206170", this::loadMobilebaseFromGist);
    }
        ...
        FxTimer.runLater(Duration.ofMillis(100), () -> {
            ...
            new Thread(() -> {
                ...
                try {
                    ...
                    Platform.runLater(() -> myGists.getItems().clear());
                    ThreadUtil.wait(20);
                    ...
                    for (GHGist gist : gists) {
                        ...
                        addFile.setOnAction(event -> new Thread(() -> Platform.runLater(() -> {
                        ...
                        Platform.runLater(() -> tmpGist.getItems().addAll(addFile,
                                                                          loadWebGist));
                            ...
                            public void handle(Event ev) {
                                ...
                                new Thread(() -> {
                                    ...
                                    for (String s : listofFiles) {
                                        ...
                                        tmp.setOnAction(event -> new Thread(() -> {
                                            try {
                                    ...
                                    Platform.runLater(() -> {
                                        tmpGist.hide();
                                        Platform.runLater(tmpGist::show);
                                    });
                                }).start();
                            }
                        };
                        ...
                        Platform.runLater(() -> myGists.getItems().add(tmpGist));
                    ...
                    for (Map.Entry<String, GHOrganization> entry : orgs.entrySet()) {
                        ...
                        Platform.runLater(() -> myOrganizations.getItems().add(OrgItem));
                    }
                    ...
                    for (GHRepository g : watching) {
                        if (ownerMenue.get(g.getOwnerName()) == null) {
                            ...
                            Platform.runLater(() -> watchingRepos.getItems().add(ownerMenue.get(g.getOwnerName())));
                        }
...
//					});
            ...
            Platform.runLater(() -> repoMenue.getItems().add(orgRepo));
        }).start();
    }
                ...
                new Thread(() ->
                           {
                               ...
                               AssetFactory.deleteFolder(cache);
                               System.exit(0);
                           }).start();
            } else
    ...
    void initialize() {
        ...
        addMarlinGCODEDevice.setOnAction(event -> Platform.runLater(ConnectionManager::onMarlinGCODE));
        loadFirmata.setOnAction(event -> Platform.runLater(ConnectionManager::onFirmata));
        new Thread(() -> {
            ...
            IGithubLoginListener listener = new IGithubLoginListener() {
                ...
                public void onLogin(String arg0) {
                    ...
                    new Thread(() -> {
                        ...
                        for (Object aSet : set) {
                            if (String.class.isInstance(aSet)) {
