...
/**
 * Sample Skeleton for "BowlerStudioMenuBar.fxml" Controller Class You can copy and paste this code
...
public class BowlerStudioMenu implements MenuRefreshEvent, INewVitaminCallback {
  ...
  @FXML private MenuItem addNewVitamin;
  ...
  private static SimpleDateFormat format =
      new SimpleDateFormat("E 'the' dd 'in' MMM-yyyy 'at' HH:mm");
  ...
  public void onMobileBaseFromGist(ActionEvent event) {
    PromptForGit.prompt(
        "Select a Creature From a Gist",
        "bcb4760a449190206170",
        (gitsId, file) -> {
  ...
  private void setToLoggedIn(final String name) {
    if (name == null) return;
    if (this.name != null && name.contentEquals(this.name)) return;
    this.name = name;
        ...
        () -> {
          ...
          new Thread() {
            public void run() {
              ...
              if (!PasswordManager.hasNetwork()) return;
  ...
  public static String gitURLtoMessage(String url) {
    while (true) {
      try {
        if (selfRef.messages.get(url) != null) break;
  ...
  public static void setUpRepoMenue(
      Menu repoMenue, String url, boolean useAddToWorkspaceItem, boolean threaded) {
  ...
  public static void setUpRepoMenue(
      Menu repoMenue, String url, boolean useAddToWorkspaceItem, boolean threaded, String message) {
    ...
    Thread t =
        new Thread() {
          public void run() {
                ...
                new Runnable() {
                  ...
                  public void run() {
                    try {
                      ...
                      Platform.runLater(
                          () -> resetMenueForLoadingFiles("Files:", orgFiles, loadFilesEvent));
                      Platform.runLater(
                          () ->
                              resetMenueForLoadingFiles("Commits:", orgCommits, loadCommitsEvent));
                      Platform.runLater(
                          () ->
                              resetMenueForLoadingFiles(
                                  "Branches:", orgBranches, loadBranchesEvent));
                ...
                event -> {
                  ...
                  Platform.runLater(
                      () -> {
                        ...
                        AddFileToGistController controller =
                            new AddFileToGistController(url, selfRef);
                        try {
                          controller.start(s);
                        } catch (Exception e) {
                          e.printStackTrace();
            ...
            Platform.runLater(
                () -> {
                  if (useAddToWorkspaceItem) orgRepo.getItems().add(addToWs);
                ...
                () -> {
                  ...
                  try {
                    ...
                    for (RevCommit commit : commitsList) {
                      ...
                      if (string.length() > 80) string = string.substring(0, 80);
                          ...
                          ev -> {
                            new Thread() {
                              public void run() {
                                ...
                                String dateString =
                                    formatSimple.format(new Date(commit.getCommitTime() * 1000L));
                                promptForNewBranch(
                                    branch + "-" + dateString,
                                      ...
                                      new Thread() {
                                        public void run() {
                                          try {
                                            ...
                                            ScriptingEngine.setCommitContentsAsCurrent(
                                                url, slugify, commit);
                                          } catch (IOException e) {
                                            exp.uncaughtException(Thread.currentThread(), e);
  ...
  private static void promptForNewBranch(
      String exampleName, String reasonForCreating, Consumer<String> resultEvent) {
    Platform.runLater(
        () -> {
      ...
      public void handle(Event event) {
        ...
        new Thread(
                () -> {
                  ...
                  newBranchItem.setOnAction(
                      event1 -> {
                        promptForNewBranch(
                            newBranchName1 + "-" + dateString,
  ...
  private static void createRepoMenuItem(
      String url, Menu orgBranches, final MenuItem onBranch, Ref r, Runnable menureset) {
    ...
    tmp.setOnAction(
        ev -> {
          new Thread() {
            ...
            private void switchToThisNewBranch(
                String url, final MenuItem onBranch, Ref select, String myName) throws IOException {
              ...
              if (myName.contentEquals(s))
                System.out.println(
                    "Changing from " + was + " to " + myName + " is now " + s + "... Success!");
  ...
  public void onLoadFile(ActionEvent e) {
    new Thread() {
      ...
      public void run() {
        ...
        if (openFile == null) openFile = ScriptingEngine.getLastFile();
        openFile =
            FileSelectionFactory.GetFile(
                openFile,
  ...
  private static void resetMenueForLoadingFiles(
      String string, Menu orgFiles, EventHandler<Event> loadFiles) {
    Platform.runLater(
        () -> {
          try {
            Platform.runLater(
      ...
      public void handle(Event ev) {
        ...
        new Thread() {
          public void run() {
            ...
            try {
              listofFiles =
                  ScriptingEngine.filesInGit(url, ScriptingEngine.getFullBranch(url), null);
  ...
  void initialize() {
    ...
    assert myGists != null
        : "fx:id=\"myGists\" was not injected: check your FXML file 'BowlerStudioMenuBar.fxml'.";
    assert myOrganizations != null
        : "fx:id=\"myOrganizations\" was not injected: check your FXML file 'BowlerStudioMenuBar.fxml'.";
    assert myRepos != null
        : "fx:id=\"myRepos\" was not injected: check your FXML file 'BowlerStudioMenuBar.fxml'.";
        ...
        try {
          ...
          HashMap<String, HashMap<String, Object>> map =
              (HashMap<String, HashMap<String, Object>>)
                  ScriptingEngine.inlineFileScriptRun(f, null);
          for (Map.Entry<String, HashMap<String, Object>> entry : map.entrySet()) {
            ...
            item.setOnAction(
                event -> {
                  loadMobilebaseFromGit(
                      (String) script.get("scriptGit"), (String) script.get("scriptFile"));
                });
            Platform.runLater(
                () -> {
    ...
    addMarlinGCODEDevice.setOnAction(
        event -> {
          Platform.runLater(() -> ConnectionManager.onMarlinGCODE());
        });
    loadFirmata.setOnAction(
        event -> {
    ...
    Thread t =
        new Thread(
            new Runnable() {
              ...
              @Override
              public void run() {
                ScriptingEngine.addIGithubLoginListener(
                    new IGithubLoginListener() {
                                  ...
                                  public void run() {
                                    ...
                                    for (int i = 0; i < set.length; i++) {
                                      ...
                                      if (String.class.isInstance(set[i])) {
                                        ...
                                        try {
                                          @SuppressWarnings("unchecked")
                                          ArrayList<String> repoFile =
                                              (ArrayList<String>) openGits.get(s);
                                          File f =
                                              ScriptingEngine.fileFromGit(
                                                  repoFile.get(0), repoFile.get(1));
    ...
    if (ScriptingEngine.hasNetwork()) t.start();
    ...
    int defSize =
        ((Number) ConfigurationDatabase.getObject("BowlerStudioConfigs", "fontsize", 12))
            .intValue();
    ...
    new Thread() {
      public void run() {
        ...
        try {
          if (vitaminsMenu == null) throw new RuntimeException("Vitamins menu was not inserted");
