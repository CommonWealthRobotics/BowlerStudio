...
public class BowlerStudio extends Application {
  static final SplashScreen splash = null; // = SplashScreen.getSplashScreen();
            ...
            while (true) {
              ...
              if (incoming.size() > 0)
                try {
                  ...
                  incoming.clear();
                  if (text != null && text.length() > 0) appendText(text);
                  text = null;
                } catch (Exception e) {
                  e.printStackTrace();
                }
  ...
  public static void main(String[] args) throws Exception {
      ...
      Platform.runLater(
          () -> {
            ...
            alert.setContentText(
                "This applications uses more that 4gb of ram\nA 32 bit JVM mode detected: "
    ...
    CSG.setProgressMoniter(
        (currentIndex, finalIndex, type, intermediateShape) -> {
          // Remove the default printing

        });
    eu.mihosoft.vrl.v3d.svg.SVGLoad.getProgressDefault();
    eu.mihosoft.vrl.v3d.svg.SVGLoad.setProgressDefault(
    ...
    if (args.length == 0) {
      ...
      if (ScriptingEngine.isLoginSuccess()) {
          ...
          firstVer =
              (String)
                  ConfigurationDatabase.getObject(
                      "BowlerStudioConfigs", "firstVersion", StudioBuildInfo.getVersion());
        } catch (Throwable t) {
        ...
        ConfigurationDatabase.setObject(
            "BowlerStudioConfigs", "currentVersion", StudioBuildInfo.getVersion());
      ...
      ConfigurationDatabase.setObject(
          "BowlerStudioConfigs", "skinBranch", StudioBuildInfo.getVersion());
      ...
      ScriptingEngine.cloneRepo(
          "https://github.com/CommonWealthRobotics/BowlerStudioVitamins.git", null);
      ...
      try {
        if (!new File(arduino).exists() && !NativeResource.isLinux()) {
          boolean alreadyNotified =
              Boolean.getBoolean(
                  ConfigurationDatabase.getObject(
                          "BowlerStudioConfigs", "notifiedArduinoDep", false)
                      .toString());
          if (!alreadyNotified) {
  ...
  //		ScriptingEngine.deleteRepo(myAssets);
  //		AssetFactory.setGitSource((String) ConfigurationDatabase.getObject("BowlerStudioConfigs",
  // "skinRepo", myAssets),
  //				StudioBuildInfo.getVersion());
  //	}
  ...
  public static void clearConsole() {
    ...
    Platform.runLater(
        () -> {
          if (getLogViewRefStatic() != null) getLogViewRefStatic().setText("");
        });
  }
  ...
  public static void setLogViewRefStatic(
      @SuppressWarnings("restriction") TextArea logViewRefStatic) {
  ...
  public void start(Stage primaryStage) {
    ...
    new Thread(
            () -> {
              ...
              try {
                mainControllerPanel =
                    AssetFactory.loadLayout("layout/BowlerStudioModularFrame.fxml");
                ...
                Platform.runLater(
                    () -> {
                      ...
                      try {
                        primaryStage
                            .getIcons()
                            .add(AssetFactory.loadAsset("BowlerStudioTrayIcon.png"));
                      } catch (Exception e) {
                        reporter.uncaughtException(Thread.currentThread(), e);
                      }
                    });
                ...
                DeviceManager.addDeviceAddedListener(
                    new IDeviceAddedListener() {
                ...
                FxTimer.runLater(
                    java.time.Duration.ofMillis((int) 2000),
                    () -> {
                      ...
                      System.out.println(
                          "Bowler-Scripting-Kernel Version: " + BowlerKernelBuildInfo.getVersion());
  ...
  public static void closeBowlerStudio() {
    ...
    new Thread() {
      ...
      public void run() {
        ...
        if (ScriptingEngine.isLoginSuccess()) ConfigurationDatabase.save();
        if (isDeleteFlag()) ScriptingEngine.deleteCache();
        System.exit(0);
      }
    }.start();
  }
  ...
  public static void print(CSG... toDisplay) {
    for (CSG c : Arrays.asList(toDisplay)) BowlerStudioController.addCsg(c);
  }

  public static void print(ArrayList<CSG> toDisplay) {
    for (CSG c : toDisplay) BowlerStudioController.addCsg(c);
  }
