...
public class ImageTracer {
  ...
  public static String imageToSVG(
      BufferedImage image, HashMap<String, Float> options, byte[][] palette) throws Exception {
    options = checkoptions(options);
  ...
  public static String imagedataToSVG(
  ...
  // in layers
  public IndexedImage imageToTracedata(
  ...
  public static IndexedImage imagedataToTracedata(
      ImageData imgd, HashMap<String, Float> options, byte[][] palette) {
    ...
    ArrayList<ArrayList<ArrayList<Integer[]>>> bps =
        batchpathscan(rawlayers, (int) (Math.floor(options.get("pathomit"))));
  ...
  public static IndexedImage colorquantization(
      ImageData imgd, byte[][] palette, HashMap<String, Float> options) {
      ...
      for (int j = 0; j < imgd.height; j++) {
        for (int i = 0; i < imgd.width; i++) {
          ...
          for (int k = 0; k < palette.length; k++) {
            ...
            // In my experience, https://en.wikipedia.org/wiki/Rectilinear_distance works better
  ...
  // 3. Batch pathscan
  public static ArrayList<ArrayList<ArrayList<Integer[]>>> batchpathscan(
      int[][][] layers, float pathomit) {
  ...
  // 4. interpolating between path points for nodes with 8 directions ( East, SouthEast, S, SW, W,
  // NW, N, NE )
  ...
  static ArrayList<ArrayList<ArrayList<Double[]>>> batchinternodes(
      ArrayList<ArrayList<ArrayList<Integer[]>>> bpaths) {
    ArrayList<ArrayList<ArrayList<Double[]>>> binternodes =
  ...
  public static ArrayList<Double[]> tracepath(
      ArrayList<Double[]> path, float ltreshold, float qtreshold) {
    int pcnt = 0, seqend = 0;
    ...
    while (pcnt < pathlength) {
      ...
      segtype2 = -1;
      seqend = pcnt + 1;
      while (((path.get(seqend)[2] == segtype1)
  ...
  public static ArrayList<Double[]> fitseq(
      ArrayList<Double[]> path, float ltreshold, float qtreshold, int seqstart, int seqend) {
    ...
    while (pcnt != seqend) {
      ...
      dist2 =
          ((path.get(pcnt)[0] - px) * (path.get(pcnt)[0] - px))
    ...
    double
        cpx =
            (((t1 * path.get(seqstart)[0]) + (t3 * path.get(seqend)[0])) - path.get(fitpoint)[0])
                / -t2,
        cpy =
            (((t1 * path.get(seqstart)[1]) + (t3 * path.get(seqend)[1])) - path.get(fitpoint)[1])
                / -t2;
  ...
  public static ArrayList<ArrayList<Double[]>> batchtracepaths(
      ArrayList<ArrayList<Double[]>> internodepaths, float ltres, float qtres) {
  ...
  public static ArrayList<ArrayList<ArrayList<Double[]>>> batchtracelayers(
    ...
    ArrayList<ArrayList<ArrayList<Double[]>>> btbis =
        new ArrayList<ArrayList<ArrayList<Double[]>>>();
  ...
  public static void svgpathstring(
      StringBuilder sb,
      String desc,
      ...
      for (int pcnt = 0; pcnt < segments.size(); pcnt++) {
        if (segments.get(pcnt)[0] == 1.0) {
          ...
          sb.append("Q ")
              ...
              .append(roundtodec((float) (segments.get(pcnt)[4] * scale), roundcoords))
              .append(" ")
              .append(roundtodec((float) (segments.get(pcnt)[5] * scale), roundcoords))
              .append(" ")
              .append(roundtodec((float) (segments.get(pcnt)[6] * scale), roundcoords))
              .append(" ");
        }
      }
  ...
  public static String getsvgstring(IndexedImage ii, HashMap<String, Float> options) {
    ...
    if (options.get("desc") != 0) {
      svgstr.append(
          "desc=\"Created with ImageTracer.java version " + ImageTracer.versionnumber + "\" ");
    }
    svgstr.append(">");
  ...
  static String tosvgcolorstr(byte[] c) {
    ...
    // stroke=\"rgb("+(c[0]+128)+","+(c[1]+128)+","+(c[2]+128)+")\" stroke-width=\"1\"
    // opacity=\""+((c[3]+128)/255.0)+"\" ";

    return "fill=\"none\" stroke=\"rgb("
        + (0)
        + ","
        + (0)
        + ","
        + (0)
        + ")\" stroke-width=\"0.354\" opacity=\""
        + 1
        + "\" ";
  }
  ...
  static ImageData blur(ImageData imgd, float rad, float del) {
    ...
    ImageData imgd2 =
        new ImageData(imgd.width, imgd.height, new byte[imgd.width * imgd.height * 4]);
