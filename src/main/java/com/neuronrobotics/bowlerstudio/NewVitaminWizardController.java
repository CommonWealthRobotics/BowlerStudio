...
public class NewVitaminWizardController extends Application {
  ...
  @FXML private TextField newTypeNameField;
  @FXML private TextField newSizeField;
            ...
            () -> {
              try {
                ...
                if (newTypeRadio.isSelected()) {
                  if (isShaft.isSelected()) Vitamins.setIsShaft(typeOfVitaminString);
                  if (isMotor.isSelected()) Vitamins.setIsActuator(typeOfVitaminString);
                  ...
                  new Thread(
                          () ->
                              BowlerStudio.createFileTab(
                                  Vitamins.getScriptFile(typeOfVitaminString)))
                      .start();
                }
    ...
    Platform.runLater(
        () ->
            nameColumn.setCellValueFactory(
                new PropertyValueFactory<MeasurmentConfig, String>("key")));
    Platform.runLater(
        () ->
            measurmentColumn.setCellValueFactory(
                new PropertyValueFactory<MeasurmentConfig, String>("measurment")));
    Platform.runLater(() -> measurmentColumn.setCellFactory(TextFieldTableCell.forTableColumn()));

    Platform.runLater(
        () -> measurmentsTable.getSelectionModel().cellSelectionEnabledProperty().set(true));
    measurmentsTable.setEditable(true);
    nameColumn.setEditable(false);
    measurmentColumn.setEditable(true);
    measurmentColumn.setOnEditCommit(
        ev -> {
          final String value =
              ev.getNewValue() != null ? (String) ev.getNewValue() : (String) ev.getOldValue();
    ...
    if (existingTypeRadio.isSelected()) {
      if (!editExisting.isSelected()) {
        ...
        for (String size : sizes) {
          if (size.contentEquals(sizeOfVitaminString)) exists = true;
        }
        if (exists) {
          Platform.runLater(
              () -> {
      ...
      new Thread(
              () -> {
                HashMap<String, Object> configsOld =
                    Vitamins.getConfiguration(
                        typeOfVitaminString, sizeComboBox.getSelectionModel().getSelectedItem());
  ...
  void onIsMotor(ActionEvent event) {
    if (isMotor.isSelected()) isShaft.setSelected(false);
  }

  @FXML
  void onIsShaft(ActionEvent event) {

    if (isShaft.isSelected()) isMotor.setSelected(false);
  }

  @FXML // This method is called by the FXMLLoader when initialization is complete
  void initialize() {
    assert x1 != null
        : "fx:id=\"x1\" was not injected: check your FXML file 'newVitaminWizard.fxml'.";
    assert x2 != null
        : "fx:id=\"x2\" was not injected: check your FXML file 'newVitaminWizard.fxml'.";
    ...
    if (typeOfVitaminString == null) typeComboBox.getSelectionModel().select(types.get(0));
    else typeComboBox.getSelectionModel().select(typeOfVitaminString);
    isShaft.setDisable(true);
    isMotor.setDisable(true);
  }
