...
public class BowlerStudioMenuWorkspace {
  ...
  public static void init(Menu workspacemenu) {
    if (workspacemenu == null) throw new RuntimeException();
  ...
  public static void add(String url, String menueMessage) {
    if (menueMessage == null || menueMessage.length() < 2) throw new RuntimeException();
    ...
    try {
      ...
      for (int i = 0; i < menu.size(); i++) {
        ...
        if (rank.get(url).intValue() != i) {
          rankChanged = true;
          System.out.println(
              "Rank changed : " + url + " to: " + i + " was: " + rank.get(url).intValue());
        }
        rank.put(url, i);
      }
      if (rankChanged) {
        Platform.runLater(
            () -> {
              if (workspaceMenu.getItems() != null) workspaceMenu.getItems().clear();

              new Thread(
                      () -> {
                        for (String url : menu) {
                          BowlerStudioMenu.setUpRepoMenue(
                              workspaceMenu,
                              url,
                              false,
                              false,
