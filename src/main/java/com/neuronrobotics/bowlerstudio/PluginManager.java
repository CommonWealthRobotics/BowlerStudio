  ...
  public PluginManager(BowlerAbstractDevice dev) {
    ...
    if (!dev.isAvailable())
      throw new RuntimeException(
          "Device is not reporting availible " + dev.getClass().getSimpleName());
  }
  ...
  public ArrayList<TitledPane> getPlugins() {
    ...
    for (DeviceSupportPluginMap c : deviceSupport) {
      if (c.getDevice().isInstance(dev)) {
        Button launcher =
            new Button(
                "Launch " + c.getPlugin().getSimpleName(),
                AssetFactory.loadIcon("Plugin-Icon.png"));
    ...
    plugins.add(info);
    if (dev.getConnection() != null) plugins.add(protocol);
