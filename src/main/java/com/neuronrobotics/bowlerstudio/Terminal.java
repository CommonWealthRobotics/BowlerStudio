...
/**
 * Sample Skeleton for "Terminal.fxml" Controller Class You can copy and paste this code into your
 * favorite IDE
 */
public class Terminal {
  ...
  // complete
  void initialize() {
    assert executionBox != null
        : "fx:id=\"executionBox\" was not injected: check your FXML file 'Terminal.fxml'.";
    assert langaugeIcon != null
        : "fx:id=\"langaugeIcon\" was not injected: check your FXML file 'Terminal.fxml'.";
    assert langauges != null
        : "fx:id=\"langauges\" was not injected: check your FXML file 'Terminal.fxml'.";
    assert outputBox != null
        : "fx:id=\"outputBox\" was not injected: check your FXML file 'Terminal.fxml'.";
    BowlerStudio.setLogViewRefStatic(outputBox);
    // Initialize your logic here: all @FXML variables will have been
    // injected
    langauges.getItems().clear();
    executionBox.setOnAction(
        event -> {
          new Thread() {
            public void run() {
    ...
    executionBox.addEventFilter(
        ...
        event -> {
          ...
          if ((event.getCode() == KeyCode.UP || event.getCode() == KeyCode.DOWN)) {
            ...
            if (event.getCode() == KeyCode.UP) historyIndex++;
            else historyIndex--;
            if (!history.isEmpty()) {
    ...
    try {
      icon =
          AssetFactory.loadAsset(
              "Script-Tab-" + langauges.getSelectionModel().getSelectedItem() + ".png");
    ...
    langauges.setOnAction(
        event -> {
          try {

            langaugeIcon.setImage(
                AssetFactory.loadAsset(
                    "Script-Tab-" + langauges.getSelectionModel().getSelectedItem() + ".png"));
          } catch (Exception e1) {
            // TODO Auto-generated catch block
          ...
          public void run() {
            ...
            try {
              ScriptingEngine.inlineScriptStringRun(
                  code, null, langauges.getSelectionModel().getSelectedItem());
              reset();
