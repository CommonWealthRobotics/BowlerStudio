...
public class Terminal implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        executionBox.setOnAction(event -> new Thread(this::startStopAction).start());
        ...
        executionBox.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
            if ((event.getCode() == KeyCode.UP || event.getCode() == KeyCode.DOWN)) {
                ...
                if (historyIndex == 0) {
                    ...
                    if (text.length() > 0)
                        history.add(text); // store what was in the box into he history
                }
                ...
                if (!history.isEmpty()) {
                    ...
                    if (historyIndex > 0)
                        Platform.runLater(() -> executionBox.setText(history.get(history.size() - historyIndex)));
                    else
                        Platform.runLater(() -> executionBox.setText(""));
                }
                event.consume();
            }
        });
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> BowlerKernel.writeHistory(history)));
        ...
        langauges.setOnAction(event -> {
            try {
                langaugeIcon.setImage(AssetFactory.loadAsset("Script-Tab-" + langauges.getSelectionModel().getSelectedItem() + ".png"));
            } catch (Exception e1) {
                e1.printStackTrace();
            }
        });
    ...
    private void start(String code) {
        ...
        scriptRunner = new Thread(() -> {
            try {
                ScriptingEngine.inlineScriptStringRun(code, null, langauges.getSelectionModel().getSelectedItem());
                reset();
