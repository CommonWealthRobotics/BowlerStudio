...
public class DeviceSupportPluginMap implements PluginFactory {
  ...
  private void setDevice(Class<?> device) {
    if (BowlerAbstractDevice.class.isAssignableFrom(device)
        || IPidControlNamespace.class.isAssignableFrom(device)) this.device = device;
    else
  ...
  private void setPlugin(Class<?> plugin) {
    if (AbstractBowlerStudioTab.class.isAssignableFrom(plugin)) this.plugin = plugin;
    else throw new RuntimeException("Plugins must subclass AbstractBowlerStudioTab");
  }
  ...
  public AbstractBowlerStudioTab generateNewPlugin()
    ...
    if (factory != null) return factory.generateNewPlugin();
    return (AbstractBowlerStudioTab)
        Class.forName(plugin.getName())
            .cast(
                plugin.newInstance() // This is where the new tab allocation is called
                );
  }
}
