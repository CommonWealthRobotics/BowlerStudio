...
public class RpcCommandPanel extends JPanel implements ActionListener {
  ...
  public RpcCommandPanel(
      RpcEncapsulation rpc, BowlerAbstractDevice device, CheckBoxTreeItem<?> rpcDhild) {
  ...
  public void actionPerformed(ActionEvent arg0) {
    ...
    for (int i = 0; i < values.length; i++) {
      if (rpc.getDownstreamArguments()[i] == BowlerDataType.ASCII) values[i] = tx.get(i).getText();
      else values[i] = Integer.parseInt(tx.get(i).getText());
    }
    BowlerDatagram bd = device.send(rpc.getCommand(values));
    rxRpc.setText(bd.getRPC());
    Object[] up = rpc.parseResponse(bd);
    for (int i = 0; i < up.length; i++) {
      if (up[i] != null) rx.get(i).setText(up[i].toString());
      else {
