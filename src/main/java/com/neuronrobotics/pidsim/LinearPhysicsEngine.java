...
public class LinearPhysicsEngine extends NonBowlerDevice {
  ...
  public void setTorque(double torque) throws Exception {
    ...
    if (torque > 0) this.torque = getMaxTorque();
    else this.torque = getMaxTorque() * -1;
    throw new Exception(
        "Max Torque exceded, actuator saturates at: "
            + maxTorque
            + " N*M, tried to set to value: "
            + torque);
  }
    ...
    new Thread() {
      public void run() {
        ...
        while (run) {
          ...
          double tGravity =
              (getLinkLen() * Math.cos(angle)) * ((getMass() * -9.8)); // the torque due to gravity
  ...
  public PIDSim getPid() {
    if (pid == null) setPid(new PIDSim(this));
    return pid;
  }
