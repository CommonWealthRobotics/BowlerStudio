      ...
      for (RotationOrder ro : list) {
        ...
        for (int i = 0; i < iterations; i++) {
          ...
          System.out.println(
              "Testing Quaturnion \nrotation "
                  + "\n     qw "
                  + oldRot.getRotationMatrix2QuaturnionW()
                  + "\n     qx "
                  + oldRot.getRotationMatrix2QuaturnionX()
                  + "\n     qy "
          ...
          assertArrayEquals(
              new double[] {Math.toRadians(rotationAngleDegrees), 0, 0},
              new double[] {
                oldRot.getRotationAzimuth(), oldRot.getRotationElevation(), oldRot.getRotationTilt()
              },
              0.001);
    ...
    for (RotationConvention conv : conventions) {
      for (RotationOrder ro : list) {
        ...
        for (int i = 0; i < iterations; i++) {
          ...
          // Convert to Affine and back
          RotationNR oldRot =
              TransformFactory.csgToNR(TransformFactory.nrToCSG(new TransformNR(0, 0, 0, newRot)))
                  .getRotation();
          double[][] rotationMatrix = newRot.getRotationMatrix();
          System.out.println(
              "\n\nTesting pure azumeth \nrotation "
    ...
    for (RotationConvention conv : conventions) {
      ...
      for (RotationOrder ro : list) {
        ...
        for (int i = 0; i < iterations; i++) {
          ...
          // Check the old rotation against the known value
          assertArrayEquals(
              new double[] {0, Math.toRadians(rotationAngleDegrees), 0},
              new double[] {
                oldRot.getRotationAzimuth(), oldRot.getRotationElevation(), oldRot.getRotationTilt()
              },
              0.001);
